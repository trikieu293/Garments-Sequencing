...
e_squared = model.addVars(eIndex, vtype=GRB.CONTINUOUS, name = "e_squared")     # Squared errors between real and expected workload up to sequence t
sse = model.addVar(vtype=GRB.CONTINUOUS, name = "sse")                          # Sum of squared deviation = sum of squared errors 
f = model.addVars(eIndex, vtype=GRB.INTEGER, name = "f")                        # Sequence t is used or not
### Objective function
...
for t in range(T):
    model.addConstr(e[t] == t*totalQuantile*n_inv - sum(sum(x[i,s]*getQuantity(i) for i in range(I)) for s in range(t)), name = "C15")
...
df["sequence"] = xResult.loc[xResult["value"] == 1, "t"].values + 1
print(df)
df.to_csv("cmse.csv", encoding='utf-8')
plotGap(model._data)
