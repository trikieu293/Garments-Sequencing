...
totalQuantity = sum(getQuantity(i) for i in range(I))       # Total quantity of all SKUs in a batch
...
e = model.addVars(eIndex, lb = -GRB.INFINITY, vtype=GRB.CONTINUOUS, name = "e") # Errors between real and expected workload up to sequence t
f = model.addVars(eIndex, vtype=GRB.INTEGER, name = "f")                        # There is a sequence t or not


### Objective function
model.setObjective(sum(e[t] for t in range(T)), GRB.MINIMIZE)
model.update()

### Constraints
## Constraints in starting model (.doc file)
for i in range(I):
    model.addConstr(sum(x[i,t] for t in range(T)) == 1, name = "Constraints 1")
for t in range(T):
    model.addConstr(sum(x[i,t] for i in range(I)) <= C, name = "Constraints 2")
    model.addConstr(sum(y[g,t] for g in range(G)) <= 1, name = "Constraints 3")
    for i in range(I):
        model.addConstr(x[i,t] <= y[getGroup(i),t], name = "Constraints 4")
        model.addConstr(x[i,t] == z[getGarment(i),t], name = "Constraints 6")
for r in range(R):        
    model.addConstr(sum(z[r,t] for t in range(T)) == 1, name = "Constraints 5")
...
for t in range(T):
    model.addConstr(e[t] == sum(x[i,t]*getQuantity(i) for i in range(I)) - totalQuantity/T, name = "C15")
    model.addConstr(sum(x[i,t]*getQuantity(i) for i in range(I)) - totalQuantity/T <= e[t] + f[t]*totalQuantity, name = "C16")
    model.addConstr(-sum(x[i,t]*getQuantity(i) for i in range(I)) + totalQuantity/T <= e[t] + f[t]*totalQuantity, name = "C17")
    model.addConstr(f[t] <= sum(x[i,t] for i in range(I)), name = "C18")
...
print(df)
df.to_csv("mae.csv", encoding='utf-8')
plotGap(model._data)
